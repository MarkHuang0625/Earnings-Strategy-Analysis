...
# Core idea:
# 1. Compute a pre-earnings signal (risk-adjusted return over past N days)
# 2. Generate long/short signals based on threshold
# 3. Execute trades using intraday 5-min bars
# 4. Apply stop-loss / profit-target rules
# 5. Compare against market benchmark (SPY)
...
def compute_pre_earnings_score(daily_df: pd.DataFrame, event_date: pd.Timestamp, lookback_days: int = 10) -> Optional[float]:
    ...
    if vol is None or np.isnan(vol) or vol == 0:
        return None
    # Signal = cumulative return / (volatility * sqrt(window))
    # This is similar to a Sharpe-like normalization
    return float(cum_ret / (vol * np.sqrt(lookback_days)))


def classify_trade(score: float, threshold: float) -> int:
    if score > threshold:
        return 1
    if score < -threshold:
        return -1
...
class EarningsStrategyAnalyzer:
    ...
    def backtest(self, ticker: str, earnings_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in earnings_df.iterrows():
            ...
            score = compute_pre_earnings_score(daily, event_date, lookback_days=LOOKBACK_DAYS)
            ...
            for threshold in THRESHOLDS:
                side = classify_trade(score, threshold)
                if side == 0:
                    continue
                signal_count += 1
                ...
                if session_type == "during":
                    ...
                    entry_ts = pd.Timestamp(
                        datetime.combine(entry_date.date(), datetime.strptime("09:30:00", "%H:%M:%S").time())
                    )
...
def run_analysis():
    ...
    for ticker in TICKERS:
        ...
        print(trades[[
            ...
            "stock_ret", "market_ret", "excess_ret", "hold_type", "hold_value",
            "threshold", "stop_loss", "profit_target", "exit_reason", "side_label"
        ]].head(10))
